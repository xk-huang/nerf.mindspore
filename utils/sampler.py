...
def sample_grid_2d(cap_h, cap_w, cap_n):
    ...
    select_indexes = np.random.choice(coords.shape[0], size=[cap_n], replace=False)  # (N, 2)
    # Sample N cells among the mesh grid
    select_coords = coords[select_indexes.tolist()].astype("int32")  # (N, 2)
...
def sample_along_rays(near, far, cap_cap_n_samples, lin_disp=False, perturb=True):
    ...
    t_vals = md.numpy.linspace(0.0, 1.0, num=cap_cap_n_samples)  # (cap_n_samples)
    if not lin_disp:
    ...
    z_vals = expand_op(z_vals)  # (cap_n_rays, cap_n_samples)
    ...
    return z_vals  # (cap_n_rays, cap_n_samples)
...
def sample_pdf(bins, weights, cap_cap_n_samples, det=False):
    ...
    indexes_g = md.ops.Stack(axis=-1)([below, above])  # (batch, cap_n_samples, 2)
